function addCommas(s) {
  ...
  function t1format(x, mult = false, y) {
    ...
    if (x >= 10) t1f = t1ones[x % 10] + t1tens[Decimal.floor(x / 10) % 10] + t1hunds[Decimal.floor(x / 100)]
    return t1f
  }
  ...
  function t2format(x, mult = false, y) {
    ...
    let t2t = t2tens[Decimal.floor(x / 10) % 10]
    if (x % 100 == 10) t2t = 'Vec'
    if (x >= 10) t2f = t2ones[x % 10] + t2t + t2hunds[Decimal.floor(x / 100)]
    return t2f
  }
  ...
  function t3format(x, mult = false, y, z) {
    ...
    let t3t = t3tens[Decimal.floor(x / 10) % 10]
    let t3h = t3hunds[Decimal.floor(x / 100)]
    if (x % 100 == 0) t3h += 'T'
  ...
  function t5format(x, mult = false, y, z) {
    let t5ills = ["", "GCT", "AAT", "TGC", "GAC", "GAG", "TTT", "GGG", "CAC", "ATT"]
    let t5ones = ["", "eN", "oD", "tR", "tE", "pT", "eX", "zE", "yO", "xN"]
    let t5tns = ["Dk", "Hn", "Dok", "TrD", "TeD", "PeD", "ExD", "ZeD", "YoD", "NeD"]
    let t5to = ["k", "k", "c", "c", "c", "k", "k", "c", "k", "c"]
    if (mult && y > 0 && x < 10) t5ones = ["", "", "D", "Tr", "T", "P", "Ex", "Z", "Y", "N"]
    let t5tens = ["", "", "I", "Tr", "Te", "P", "E", "Z", "Y", "N"]
    let t5hunds = ["", "Ho", "Do", "Tro", "To", "Po", "Ex", "Zo", "Yo", "No"]
    let t5f = t5ills[x]
    if ((mult && y > 0) || z >= 1e3) t5f = t5ones[x]
    let t5t = t5tens[Decimal.floor(x / 10) % 10]
    let t5h = t5hunds[Decimal.floor(x / 100)]
    if (x % 100 == 0) t5h += 'T'
    if (x % 100 < 20 && x % 100 > 9) t5t = t5tns[x % 10]
    if (x % 100 > 19) t5t += t5to[x % 10] + t5ones[x % 10]
    if (x >= 10) t5f = t5h + t5t
    return t5f
  }
  ...
  function standard(decimal, precision) {
    ...
    let illion = decimal.log(10).div(3).floor().sub(1)
    ...
    let st = t1format(t1)
    if (illion.gte(1e3)) st = t1format(Decimal.floor(t1 / 1e6), true, t2) + t2format(t2) + ((Decimal.floor(t1 / 1e3) % 1e3 > 0) ? ('-' + t1format(Decimal.floor(t1 / 1e3) % 1e3, true, t2 - 1) + t2format(t2 - 1)) : '')
    if (illion.gte(1e6)) st += ((t1 % 1e3 > 0) ? ('-' + t1format(t1 % 1e3, true, t2 - 2) + t2format(t2 - 2)) : '')
    if (t2illion.gte(1e3)) st = t2format(Decimal.floor(t2 / 1e6), true, t3) + t3format(t3) + ((Decimal.floor(t2 / 1e3) % 1e3 > 0) ? ("a'-" + t2format(Decimal.floor(t2 / 1e3) % 1e3, true, t3 - 1) + t3format(t3 - 1)) : '')
    if (t2illion.gte(1e6)) st += ((t2 % 1e3 > 0) ? ("a'-" + t2format(t2 % 1e3, true, t3 - 2) + t3format(t3 - 2)) : '')
    if (t3illion.gte(1e3)) st = t3format(Decimal.floor(t3 / 1e6), true, t4) + t4format(t4, Decimal.floor(t3 / 1e6)) + t5format(t5, Decimal.floor(t4 / 1e6)) + ((Decimal.floor(t3 / 1e3) % 1e3 > 0) ? ("-" + t3format(Decimal.floor(t3 / 1e3) % 1e3, true, t4 - 1, t3) + t4format(Decimal.floor(t4 / 1e3) % 1e3, true, t5 - 1, t4) + t5format(t5 - 1, Decimal.floor(t4 / 1e3) % 1e3)) : '')
    if (t3illion.gte(1e6)) st += ((t3 % 1e3 > 0) ? ("-" + t3format(t3 % 1e3, true, t4 - 2, t3) + t4format(t4 % 1e3, true, t5 - 2, t4) + t5format(t5 - 2, t4 % 1e3)) : '')
  ...
  function standardV2(number) {
    var ones = ["", "K", "M", "B", "T", "Qa", "Qi", "Sx", "Sp", "Oc", "No"];
    var additioner = ["", "U", "D", "T", "Qa", "Qi", "Sx", "Sp", "Oc", "No"];
    var tens = ["", "Dc", "Vg", "Tg", "Qag", "Qig", "Sxg", "Spg", "Ocg", "Nog"];
    
    var magnitude = Math.floor(Math.log10(number) / 3);
    
    var formattedNumber = (number / Math.pow(10, magnitude * 3)).toFixed(2);
    
    var result;
    
    if (magnitude <= 10) {
      result = formattedNumber + ones[magnitude];
    } else if (magnitude % 10 === 1) {
      result = formattedNumber + additioner[Math.floor(magnitude / 10)] + " " + tens[1];
    } else {
      result = formattedNumber + additioner[Math.floor(magnitude / 10)] + " " + tens[magnitude % 10] + " " + ones[magnitude % 10];
    }
  ...
  function prismSQFormat(decimal, precision = 4) {
    decimal = new Decimal(decimal)
    let s = Decimal.slog(decimal).pow(0.01)
    let mag = s.sub(s.floor()).pow10().pow10()
    let m = commaFormat(mag, precision)
    if (mag.gte(1e6)) m = commaFormat(mag, 0)
    let e = Decimal.add(commaFormat(s.floor().sub(1), 0), 1)
    if (s.gte(1e9)) e = formatWhole(s.floor())
    if (decimal.layer >= 1e10) return prismSQFormat(s, precision) + '^2'
    if (decimal.layer >= 1e9) return 'E1#' + e
    if ((decimal.layer > 0 && decimal.mag > 0) || decimal.mag >= 1e10) return 'Prism^' + m + '^' + e
    if (decimal.mag >= 1e3) return commaFormat(decimal, 0)
    if (decimal.mag >= 0.001) return regularFormat(decimal, precision)
    if (decimal.mag >= 1 / 9e15) return '1/' + standard(decimal.recip(), precision)
    if (decimal.sign != 0) return '1/' + hyperEformat(decimal.recip(), precision)
  ...
  function letter(decimal, precision, str) { //AD NG+++
    ...
    if (decimal.gte(1000)) return m.toStringWithDecimalPlaces(precision) + ' ' + ret
    if (decimal.mag >= 1e3) return commaFormat(decimal, 0)
    if (decimal.mag >= 0.001) return regularFormat(decimal, precision)
  ...
  function format(decimal, precision = 3) {
    ...
    else if (player.notation == 'Mass Unit') {
      return massStadard(decimal, precision)
    }
    else if (player.notation == 'Prism SQ') {
      return prismSQFormat(decimal, precision)
    }
    else if (player.notation == 'Hex') {
      return toHexadecimal(decimal, precision)
    }
    else if (player.notation == 'Test') {
      return standardV2(decimal, precision)
    }
  ...
  function verseTime(years) {
    s = Decimal.slog(new Decimal(years)).sub(Decimal.log10(9))
    let verse1 = [2, 3, 4, 5]
    ...
    let mag = Decimal.slog(Decimal.add(years, -verse1[id] + 1)).div(1e9)
  ...
  function masst1format(x, mult = false, y) {
    let ills = ['', 'Kg', 't', 'Mg', 'Gg', 'Tg', 'Pg', 'Eg', 'Zg', 'Yg', 'Xg', 'Vg', 'Qg', "Ptg", "Hg", "Htg", "Ogg"]
    let t1ones = ["", "K", "t", "M", "G", "T", "P", "E", "Z", "Y", "X", "V", "Q", "PT", "H", "HT", "OG"]
    if (mult && y > 0 && x < 17) t1ones = ["", "K", "t", "M", "G", "T", "P", "E", "Z", "Y", "X", "V", "Q", "PT", "H", "HT", "OG"]
    let t1tens = ["", "KM", "tM", "MM", "GM", "TM", "PM", "EM", "ZM", "YM", "XM", "VM", "QM", "PTM", "HM", "HTM", "OGM"]
    let t1hunds = ["", "SK", "St", "SM", "SG", "ST", "SP", "SE", "SZ", "SY", "SX", "SV", "SQ", "SPT", "SH", "SHT", "SOG"]
    let t1f = ills[x]
    if (mult && y > 0) t1f = t1ones[x]
    if (x >= 17) t1f = t1ones[x % 17] + t1tens[Math.floor(x / 17) % 17] + t1hunds[Math.floor(x / 289)]
    return t1f
  }
  
  function masst2format(x, mult = false, y) {
    let t2ills = ["", "K", "t", "M", "G", "T", "P", "E", "Z", "Y"]
    let t2ones = ["", "Me", "Du", "Tr", "Te", "Pe", "He", "Hp", "Ot", "En"]
    if (mult && y > 0 && x < 10) t2ones = ["", "", "Mc", "Na", "Pc", "Fm", "At", "Zp", "Yc", "Xn"]
    let t2tens = ["", "c", "Ic", "TCn", "TeC", "PCn", "HCn", "HpC", "OCn", "ECn"]
    let t2hunds = ["", "Hc", "DHe", "THt", "TeH", "PHc", "HHe", "HpH", "OHt", "EHc"]
  ...
  function masst3format(x, mult = false, y, z) {
    let t3ills = ["", "Kl", "Mg", "Gi", "Ter", "Pt", "Ex", "Zt", "Yt", "Xe"]
    let t3ones = ["", "eN", "oD", "tR", "tE", "pT", "eX", "zE", "yO", "xN"]
    let t3tns = ["Dk", "Hn", "Dok", "TrD", "TeD", "PeD", "ExD", "ZeD", "YoD", "NeD"]
    let t3to = ["k", "k", "c", "c", "c", "k", "k", "c", "k", "c"]
  ...
  function massStadard(decimal, precision) {
    decimal = new Decimal(decimal)
    ...
    let st = masst1format(t1)
    if (illion.gte(1e3)) st = masst1format(Math.floor(t1 / 1e6), true, t2) + masst2format(t2) + ((Math.floor(t1 / 1e3) % 1e3 > 0) ? ('-' + masst1format(Math.floor(t1 / 1e3) % 1e3, true, t2 - 1) + masst2format(t2 - 1)) : '')
    if (illion.gte(1e6)) st += ((t1 % 1e3 > 0) ? ('-' + masst1format(t1 % 1e3, true, t2 - 2) + masst2format(t2 - 2)) : '')
    if (t2illion.gte(1e3)) st = masst2format(Math.floor(t2 / 1e6), true, t3) + masst3format(t3) + ((Math.floor(t2 / 1e3) % 1e3 > 0) ? ("a'-" + masst2format(Math.floor(t2 / 1e3) % 1e3, true, t3 - 1) + masst3format(t3 - 1)) : '')
    if (t2illion.gte(1e6)) st += ((t2 % 1e3 > 0) ? ("a'-" + masst2format(t2 % 1e3, true, t3 - 2) + masst3format(t3 - 2)) : '')
    if (t3illion.gte(1e3)) st = masst3format(Math.floor(t3 / 1e6), true, t4) + masst4format(t4, Math.floor(t3 / 1e6)) + ((Math.floor(t3 / 1e3) % 1e3 > 0) ? ("`-" + masst3format(Math.floor(t3 / 1e3) % 1e3, true, t4 - 1, t3) + masst4format(t4 - 1, Math.floor(t3 / 1e3) % 1e3)) : '')
    if (t3illion.gte(1e6)) st += ((t3 % 1e3 > 0) ? ("`-" + masst3format(t3 % 1e3, true, t4 - 2, t3) + masst4format(t4 - 2, t3 % 1e3)) : '')
    if (decimal.mag >= 1e3 || (decimal.layer > 0 && decimal.mag >= 0)) return m + st
    if (decimal.mag >= 1e3) return commaFormat(decimal, 0)
    if (decimal.mag >= 0.001) return regularFormat(decimal, precision)
    if (decimal.sign != 0) return '1/' + standard(decimal.recip(), precision)
  ...
  function toHexadecimal(s, shorten = false) {
    let number = new Decimal(s)
    const hexDigits = "0123456789ABCDEF";
    let result = "";
    let base = new Decimal(16);
  
    while (number > 0) {
      result = hexDigits.charAt(number % base) + result;
      number = Decimal.floor(Decimal.div(number, base));
  
      // Increase the base if it runs out of digits
      if (number.gte(1) && number % base === 0) {
        base.mul(16);
      }
    }
    ...
    if (shorten) {
      ...
      const maxLength = shorten === false ? 3 : shorten;
  ...
  function hex(decimal, precision = 3) {
    decimal = new Decimal(decimal);
    let exponent = decimal.e;
  
    if (exponent) {
      let coefficient = decimal.times(Decimal.pow(10, -exponent));
      let coefficientString = coefficient.toFixed(precision);
      let parts = coefficientString.split(/[eE]/);
      let coefficientHex = (+parts[0]).toString(16);
  
      return coefficientHex + "e" + parts[1];
    }
